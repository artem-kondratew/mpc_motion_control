...
class AccTelemetry(Node):
    ...
    def _tick(self) -> None:
        ...
        # Полоса КВАЗИСТАТИЧНА (не меняется) — НЕ гейтим валидность на её свежести:
        # мост шлёт полосу ~2 Гц (раз в 500 мс) при input_timeout=0.5с -> ровно на
        # границе -> малейший джиттер -> is_valid флапает -> ACC сбрасывает v_cmd в 0,
        # робот дёргается и отстаёт. Достаточно, что полоса получена хоть раз.
        # Свежесть требуем только у ДИНАМИКИ (позы лидера/свои, скорость лидера).
        have_inputs = (
