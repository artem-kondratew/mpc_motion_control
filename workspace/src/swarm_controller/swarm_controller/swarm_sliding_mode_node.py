...
from .submodules.swarm_sliding_mode import *


class SwarmSlidingModeNode(Node):

    def __init__(self) -> None:
        super().__init__('swarm_sliding_mode_node')
        ...
        for robot_id in self.robots_ids:
            self.control_publishers[robot_id] = self.create_publisher(
    ...
    def telemetry_callback(self, msg: Telemetry):
        ...
        for agent in self.swarm.agents:
            ...
            peers_vecs = get_peers_vecs(
                self.swarm.agents, agent, self.swarm.pacemaker, self.get_logger()
            )
            best_peers_vecs, azimuth = select_peers_and_azimuth(
                peers_vecs, self.swarm.pacemaker, agent, self.get_logger()
            )
            a, w = control_cmd(
                best_peers_vecs, azimuth, agent, self.gap, self.acc_max, self.kp_theta,
                self.get_logger()
            )
            ...
            #     f'agent {agent.id}: dt={dt:.3f} a={a:.3f} w={w:.3f} v={twist.linear.x:.3f}'
            # )
