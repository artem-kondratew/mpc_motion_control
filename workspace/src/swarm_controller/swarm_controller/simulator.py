...
class Simulator(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameters(namespace='', parameters=[
            ...
            ('pacemaker_idx', 1),
            ('noise_std', 0.02),
            # plant: 2nd-order Newton + lumped drive with back-EMF damping.
            # Matches the model derived on defense slides:
            #   m·v̇ = F − F_C·sign(v)
            #   τ_F·Ḟ = α·v_cmd − β·v − F
            # where α and β differ due to back-EMF damping inside the
            # closed-loop response between v_cmd (CAN command) and v
            # (encoder reading).  PID inside STM32 is NOT modelled here —
            # we use the identified lumped response instead.
            ('m', 2.0),       # robot mass [kg]
            ('F_C', 0.94),    # Coulomb friction [N], identified via b2 coast-down
            ('alpha', 21.0),  # drive gain on v_cmd [N·s/m], identified step-response
            ('beta', 19.0),   # damping on v [N·s/m] — calibrated for v_real=v_cmd_max
            ('tau_F', 0.15),  # effective mechanical lag [s]
            # angular axis: 1st-order lag from w_cmd to w. Identified on
            # hardware via step response (±1 rad/s, transient ~0.1-0.15 s).
            # No 2nd-order ringing observed.
        ...
        self.F_C = self.get_parameter('F_C').value
        self.alpha = self.get_parameter('alpha').value
        self.beta = self.get_parameter('beta').value
        self.tau_F = self.get_parameter('tau_F').value
        self.tau_w = self.get_parameter('tau_w').value
        ...
        # F is the internal drive force (Newton's 2nd law: m·v̇ = F − F_C·sign(v)).
        self.states = {}
    ...
    def _step(self):
        for robot_id, state in self.states.items():
            ...
            # 2nd-order longitudinal: Newton + Coulomb friction + lumped drive
            # with back-EMF damping (asymmetric coefficients α and β).
            #   m·v̇ = F − F_C·sign(v)
            #   τ_F·Ḟ = α·v_cmd − β·v − F
            # sign(v) ≈ +1: cruise-control moves forward only.  Below a tiny
            # deadband Coulomb is zeroed to allow rest.
            coulomb = self.F_C if v > 1e-3 else 0.0
            v_dot = (F - coulomb) / self.m
            F_dot = (self.alpha * v_cmd - self.beta * v - F) / self.tau_F
            v += self.dt * v_dot
            F += self.dt * F_dot
            # Forbid reverse motion (Coulomb stop).
            if v < 0.0:
                v = 0.0

            # angular: 1st-order lag,  ẇ = (w_cmd − w) / τ_w
            w_dot = (self.w_cmd[robot_id] - w) / self.tau_w
            w += self.dt * w_dot
