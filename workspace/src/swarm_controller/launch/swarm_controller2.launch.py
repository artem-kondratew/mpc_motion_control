...
def generate_launch_description():
    ...
    controller_type_arg = DeclareLaunchArgument(
        ...
        description=(
            "Which follower controller to use:\n"
            "  'sliding'  -> Python sliding-mode (default, baseline)\n"
            "  'mpc_kin'  -> Kinematic ACC MPC (adas-style, gap-keeping only)\n"
            "  'mpc_full' -> Kinematic ACC MPC + lateral MPC (path-following)"
        ),
    )
    ...
    # peer_localization always runs — publishes Telemetry that all controllers consume.
    ...
    # 2) Kinematic ACC MPC follower (gap-keeping only, no path-following)
    kin_mpc_node = Node(
        package=package_name,
    ...
    # 3) Full = kinematic ACC MPC publishing to /long_cmd, plus lateral MPC
    #    that subscribes to /long_cmd + /pacemaker/path + /odom and
    #    publishes the merged Twist to /cmd_vel.
    full_long_node = Node(
        package=package_name,
        executable='swarm_acc_mpc_node',
        parameters=[
