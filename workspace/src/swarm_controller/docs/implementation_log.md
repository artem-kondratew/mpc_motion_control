...
Связанные документы:
- [`mpc_math.md`](mpc_math.md) — математика кинематического ACC/CC/Lat MPC.
- [`future_work.md`](future_work.md) — отложенные улучшения.

> **Note (2026-05-24):** все Newton/dynamic варианты MPC удалены — остался
> только кинематический стек (SwarmAccController, SwarmCruiseController,
> SwarmLatController + sliding mode). Исторические упоминания
> `swarm_acc_kin_mpc.py`, `SwarmKinAccController`, `swarm_unified_*` и
> `swarm_acc_mpc.md` ниже относятся к удалённым артефактам и сохранены как
> хронология решений.
